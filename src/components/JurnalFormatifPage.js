        ...
        React.createElement('div', { className: "fixed inset-0 bg-black bg-opacity-60 z-[70] flex items-center justify-center p-4" },
            React.createElement('div', { className: "bg-white rounded-xl shadow-xl w-full max-w-lg flex flex-col max-h-[90vh]" },
                React.createElement('div', { className: "p-4 border-b flex-shrink-0" },
                    React.createElement('h3', { className: "text-lg font-bold text-zinc-800" }, isEditing ? 'Edit Catatan Formatif' : 'Tambah Catatan Baru'),
                    React.createElement('p', { className: "text-sm text-zinc-500" }, `Untuk: ${studentName}`)
                ),
                React.createElement('div', { className: "p-6 space-y-4 overflow-y-auto" },
                    React.createElement('div', { className: "grid grid-cols-1 md:grid-cols-2 gap-4" },
                        React.createElement('div', null,
                            React.createElement('label', { htmlFor: 'date', className: "block text-sm font-medium text-zinc-700" }, "Tanggal"),
                            React.createElement('input', { type: "date", name: "date", id: "date", value: noteData.date || '', onChange: handleChange, className: "mt-1 block w-full p-2 border border-zinc-300/60 rounded-lg text-sm" })
                        ),
                        React.createElement('div', null,
                            React.createElement('label', { htmlFor: 'type', className: "block text-sm font-medium text-zinc-700" }, "Jenis Asesmen"),
                            React.createElement('select', { name: "type", id: "type", value: noteData.type || '', onChange: handleChange, className: "mt-1 block w-full p-2 border border-zinc-300/60 rounded-lg text-sm" },
                    ...
                    React.createElement('div', { className: "space-y-3 pt-2 border-t border-slate-100" },
                        React.createElement('p', { className: "text-xs font-semibold text-zinc-500 uppercase tracking-wider" }, "Konteks Pembelajaran (Opsional)"),
                        
                        React.createElement('div', null,
                            React.createElement('label', { htmlFor: 'subjectId', className: "block text-sm font-medium text-zinc-700" }, "Mata Pelajaran"),
                            React.createElement('select', { name: "subjectId", id: "subjectId", value: noteData.subjectId || '', onChange: handleChange, className: "mt-1 block w-full p-2 border border-zinc-300/60 rounded-lg text-sm" },
                        ...
                        noteData.subjectId && (
                            React.createElement('div', null,
                                React.createElement('label', { htmlFor: 'slmId', className: "block text-sm font-medium text-zinc-700" }, "Lingkup Materi (SLM)"),
                                React.createElement('select', { name: "slmId", id: "slmId", value: noteData.slmId || '', onChange: handleChange, className: "mt-1 block w-full p-2 border border-zinc-300/60 rounded-lg text-sm" },
                        ...
                        noteData.slmId && (
                            React.createElement('div', null,
                                React.createElement('label', { htmlFor: 'tpId', className: "block text-sm font-medium text-zinc-700" }, "Tujuan Pembelajaran (TP)"),
                                React.createElement('select', { name: "tpId", id: "tpId", value: noteData.tpId || '', onChange: handleChange, className: "mt-1 block w-full p-2 border border-zinc-300/60 rounded-lg text-sm" },
                        ...
                        React.createElement('div', null,
                            React.createElement('label', { htmlFor: 'topic', className: "block text-sm font-medium text-zinc-700" }, "Topik Lainnya / Catatan Tambahan"),
                            React.createElement('input', { type: "text", name: "topic", id: "topic", value: noteData.topic || '', onChange: handleChange, className: "mt-1 block w-full p-2 border border-zinc-300/60 rounded-lg text-sm", placeholder: "Contoh: Perilaku di Kantin, Kebersihan, dll." })
                        )
                    ),

                    React.createElement('div', {className: "pt-2 border-t border-slate-100"},
                        React.createElement('label', { htmlFor: 'note', className: "block text-sm font-medium text-zinc-700" }, "Catatan/Observasi"),
                        React.createElement('textarea', { name: "note", id: "note", value: noteData.note || '', onChange: handleChange, rows: "5", className: "mt-1 block w-full p-2 border border-zinc-300/60 rounded-lg text-sm", placeholder: "Tuliskan observasi Anda di sini..." })
                    )
                ),
    ...
    return (
        React.createElement(React.Fragment, null,
            ...
            React.createElement('div', { className: "fixed inset-0 bg-black bg-opacity-60 z-[60] flex items-center justify-center p-4" },
                React.createElement('div', { className: "bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col" },
                    React.createElement('div', { className: "flex justify-between items-center p-4 border-b bg-[#fafafa] rounded-t-lg" },
                        React.createElement('div', null,
                        ...
                        notes && notes.length > 0 ? (
                            React.createElement('div', { className: "space-y-4" },
                                notes.map(note => {
                                    ...
                                    return (
                                    React.createElement('div', { key: note.id, className: "p-4 border border-zinc-200/60 rounded-xl bg-white shadow-sm hover:shadow-sm transition-shadow" },
                                        React.createElement('div', { className: "flex justify-between items-start mb-3" },
                                            React.createElement('div', null,
                                                React.createElement('div', { className: "flex items-center gap-2 mb-1" },
                                                    React.createElement('span', { className: "px-2 py-0.5 rounded text-xs font-bold bg-zinc-200 text-zinc-700" }, new Date(note.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })),
                                                    React.createElement('span', { className: "px-2 py-0.5 rounded text-xs font-semibold bg-zinc-100 text-zinc-800" }, note.type),
                                                    subjectLabel && React.createElement('span', { className: "px-2 py-0.5 rounded text-xs font-semibold bg-emerald-100 text-emerald-800" }, subjectLabel)
                                                ),
                                                (note.topic || note.tpId) && (
                                                    React.createElement('p', { className: "text-sm font-semibold text-zinc-800 mt-1" }, 
                                                        note.topic ? note.topic : (note.tpId ? (note.tpId.length > 100 ? note.tpId.substring(0, 100) + '...' : note.tpId) : '')
                                                    )
                                                )
                                            ),
                                            React.createElement('div', { className: "flex gap-2" },
                                                React.createElement('button', { onClick: () => handleEdit(note), className: "text-xs font-medium text-zinc-900 hover:bg-zinc-50 px-2 py-1 rounded" }, "Edit"),
                                        ...
                                        React.createElement('div', { className: "text-sm text-zinc-700 whitespace-pre-wrap leading-relaxed pl-2 border-l-2 border-zinc-200/60" }, note.note),
                                        (note.slmId && note.tpId) && (
                                            React.createElement('div', { className: "mt-3 pt-2 border-t border-slate-100 text-xs text-zinc-500 flex flex-col gap-0.5" },
                            ...
                            React.createElement('div', { className: "flex flex-col items-center justify-center py-12 text-zinc-500" },
                    ...
                    React.createElement('div', { className: "flex justify-end p-4 border-t bg-white rounded-b-lg" },
                        React.createElement('button', { onClick: onClose, className: "px-4 py-2 text-sm bg-zinc-100/50 text-zinc-700 border border-zinc-300/60 rounded-lg hover:bg-zinc-200 font-medium" }, "Tutup")
                    )
                )
            )
        )
    ...
    return (
        React.createElement('div', { className: "flex flex-col gap-6 space-y-0" },
            ...
            React.createElement('div', { className: "flex-shrink-0" },
                React.createElement('h2', { className: "text-3xl font-bold text-zinc-800" }, "Jurnal Formatif"),
                                ...
                                students.map((student, index) => {
                                    ...
                                    return (
                                        React.createElement('tr', { key: student.id, className: "bg-white hover:bg-[#fafafa]" },
                                            ...
                                            React.createElement('td', { className: "px-6 py-4 font-medium text-zinc-900 border-b border-zinc-200/60" }, student.namaLengkap),
                                            React.createElement('td', { className: "px-6 py-4 text-center border-b border-zinc-200/60" }, 
                                                React.createElement('span', { className: `inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-medium ${noteCount > 0 ? 'bg-zinc-100 text-zinc-800' : 'bg-zinc-100/50 text-zinc-500'}` },
                                                    noteCount
                                                )
                                            ),
                                            React.createElement('td', { className: "px-6 py-4 text-center border-b border-zinc-200/60" },
