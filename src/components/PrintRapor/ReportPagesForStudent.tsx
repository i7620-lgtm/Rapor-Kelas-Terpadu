...
export const ReportPagesForStudent: React.FC<ReportPagesForStudentProps> = ({ 
    ...
    const onUpdateDescription = React.useCallback((sid: string, subId: string, type: 'highest' | 'lowest', val: string, currentDescriptions: any) => {
        setGrades(prev => {
    ...
    const onUpdateStudent = React.useCallback((id: string, k: string, v: string) => setStudents((prev) => prev.map((s) => (s.id === id ? { ...s, [k]: v } : s))), [setStudents]);

    const onUpdateSettings = React.useCallback((k: string, v: any) => setSettings((s) => {
        const newS = { ...s, [k]: v };
    ...
    const onUpdateNote = React.useCallback((sid: string, v: string) => {
        useNotesStore.getState().updateNote(sid, v, settings?.semester);
    }, [settings?.semester]);

    const onUpdateAttendance = React.useCallback((sid: string, k: string, v: string) =>
        setAttendance((prev) => {
            const n = [...prev];
    ...
    const onUpdateExtraDescription = React.useCallback((sid: string, eid: string, v: string) =>
    ...
    const onUpdateCocurricularManual = React.useCallback((sid: string, v: string) =>
    ...
    const notesForMeasurement = useMemo(() => {
        ...
        return safeNotes;
    }, [shouldDisplayRank, student, rank, notes, currentSemester]);
