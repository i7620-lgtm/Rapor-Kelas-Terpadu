...
const Toast = ({ message, type, onClose }) => {
  ...
  const baseClasses = "fixed top-5 left-1/2 -translate-x-1/2 z-[100] flex items-center justify-between w-full max-w-xs p-4 rounded-xl shadow-md animate-fade-in-down";
  const typeClasses = {
  ...
  return (
    React.createElement('div', { className: `${baseClasses} ${typeClasses[type]}`, role: "alert" },
        React.createElement('div', { className: "flex items-center" },
            React.createElement('div', { className: `inline-flex items-center justify-center flex-shrink-0 w-8 h-8 ${iconClasses[type]} rounded-xl` },
                type === 'success' ? (
        ...
        React.createElement('button', {
            type: "button",
            className: "ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-xl focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8",
            onClick: onClose,
            "aria-label": "Close"
        },
