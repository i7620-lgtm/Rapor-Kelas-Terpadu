...
const generateSubjectDescription = (
  student,
  detailedGrade,
...
// --- Helper Component: AutoSizingTextarea ---
const AutoSizingTextarea = ({ value, onChange, onBlur, ...props }) => {
...
const SummativeModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (isSLM && item) {
        ...
        const curriculumKey = subject.curriculumKey || subject.fullName;
        const objectivesForSubject =
          objectives[gradeKey]?.[curriculumKey] || [];
  ...
  }, [isOpen, item, objectives, subject, gradeNumber, isSLM, students, grades]);
  ...
  const handleBulkGenerateDescriptions = () => {
    const gradeKey = `Kelas ${gradeNumber}`;
    const curriculumKey = subject.curriculumKey || subject.fullName;
    let currentObjectives = objectives[gradeKey]?.[curriculumKey] || [];

    if (isSLM) {
      // Replace objectives for this SLM with localObjectives
      const otherObjectives = currentObjectives.filter(
        (o) => o.slmId !== item.id,
      );
      ...
      currentObjectives = [...otherObjectives, ...thisSlmObjectives];
    }
            ...
            React.createElement(
                      ...
                      React.createElement(
                        ...
                        React.createElement(
                          ...
                          React.createElement(
                            ...
                            {
                              className:
                                "text-indigo-900 text-[10px] font-bold",
                            },
                            "BOBOT",
                          ),
                          React.createElement("input", {
                    ...
                    localObjectives.map((_, i) =>
                      React.createElement(
                        ...
                        React.createElement(
                          ...
                          {
                            className:
                              "px-2 py-2 text-center font-normal border-b",
                          },
                          "Kualitatif",
                        ),
                      ),
              ...
              React.createElement(
                ...
                relevantStudents.map((student, index) => {
                  ...
                  let average = null;
                  if (isSLM) {
                    const slmData = studentGrade.slm?.find(
                      (s) => s.id === item.id,
                    );
                          ...
                          return React.createElement(
                            ...
                            React.createElement(
                              ...
                              React.createElement(
                                ...
                                Object.keys(QUALITATIVE_DESCRIPTORS).map(
                                  (code) =>
                                    React.createElement(
                                      "option",
                                      { key: code, value: code },
        ...
        React.createElement(
          ...
          localSlms.length > 0
            ? localSlms.map((slm) =>
                React.createElement(
                  "div",
                  { key: slm.id, className: "border rounded-lg" },
                  React.createElement(
                    "div",
                    {
                      className:
                        "flex items-center p-3 bg-slate-50 rounded-t-lg border-b gap-4",
                    },
                    React.createElement("input", {
                      type: "text",
                      value: slm.name,
                      onChange: (e) =>
                        handleSlmNameChange(slm.id, e.target.value),
                      className:
                        "flex-grow text-md font-semibold text-slate-800 bg-transparent border-b-2 border-transparent focus:border-indigo-500 outline-none",
                    }),
                    React.createElement(
                      "button",
                      {
                        ...
                        className:
                          "text-red-500 hover:text-red-700 p-1 text-2xl leading-none flex-shrink-0",
                      },
                      "×",
                    ),
                  ),
                  React.createElement(
                    ...
                    slm.tps.map((tp, index) =>
                      React.createElement(
                        ...
                        React.createElement(
                          ...
                          {
                            className:
                              "font-semibold text-xs text-slate-400 pt-2",
                          },
                          `TP ${index + 1}:`,
                        ),
                        React.createElement("textarea", {
                          value: tp.text,
                          onChange: (e) =>
                            handleTpTextChange(slm.id, index, e.target.value),
                          className:
                            "flex-grow p-1.5 border rounded-md text-sm resize-none",
                          rows: 2,
                        }),
                        React.createElement(
                          "button",
                          {
                            onClick: () => handleDeleteTp(slm.id, index),
                            className:
                              "text-red-500 hover:text-red-700 p-1 text-xl leading-none flex-shrink-0 mt-1",
                          },
                          "×",
                        ),
                      ),
                    ...
                    React.createElement(
                      ...
                      {
                        className:
                          "pt-3 border-t flex flex-row flex-wrap gap-2 justify-between items-center",
                      },
                      React.createElement(
                        "div",
                      ...
                      React.createElement(
                        ...
                        React.createElement(
                          "button",
                          {
                            onClick: () => handleAddManualTp(slm.id),
                            className:
                              "px-3 py-1 text-xs font-medium text-slate-700 bg-white border border-slate-200 rounded-lg hover:bg-indigo-50 hover:text-indigo-700 hover:border-indigo-200 transition-all",
                          },
                          "+ Tambah TP Manual",
                        ),
                        React.createElement(
                          "button",
                          {
                            onClick: () => handleOpenTpSelection(slm.id),
                            className:
                              "px-3 py-1 text-xs font-medium text-indigo-700 bg-indigo-100 rounded-lg hover:bg-indigo-200",
                          },
                          "Pilih TP dari Data",
                        ),
                      ),
...
const NilaiTableView = (props) => {
  ...
  // Refs to individual SLM name spans for truncation detection
  ...
  const relevantStudents = useMemo(() => {
    ...
    // Case 1: Kepercayaan specifically
    // The ID is defined in App.js as PAKTTMYME
    if (
      subject.id === "PAKTTMYME" ||
      curriculumKey.toLowerCase().includes("kepercayaan terhadap tuhan")
    ) {
  ...
  const { slmHeaders, tpHeaders, columnKeys } = useMemo(() => {
    ...
    allSlms.forEach((slm) => {
      if (activeSlmIds.includes(slm.id)) {
        ...
        // If SLM has no TPs but is active, ensure it gets a column (colspan 1)
        if (slm.tps.length === 0) {
          tpCounter++; // Still need a dummy TP for the header to span
          const header = {
            slmId: slm.id,
            tpIndex: 0,
            text: "No TP defined",
  ...
  // Measure truncation for SLM headers
  useLayoutEffect(() => {
      ...
      React.createElement(
            ...
            slmHeaders.map((h) => {
              ...
              return React.createElement(
                "th",
                {
                  ...
                  colSpan: h.colSpan,
                  className:
                    "p-2 text-center border-b border-l border-slate-200",
              ...
              tpHeaders.map((h) =>
                React.createElement(
                  "th",
                  {
                    ...
                    className:
                      "p-1 text-center border-b border-l border-slate-200 bg-indigo-50",
                  },
                  React.createElement("input", {
                    type: "number",
                    min: 0,
                    ...
                    className: `w-full p-0.5 text-center text-[10px] border rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500 ${weights.TP?.[h.slmId]?.[h.tpIndex] !== null && weights.TP?.[h.slmId]?.[h.tpIndex] !== undefined && weights.TP?.[h.slmId]?.[h.tpIndex] !== "" ? "border-green-500 ring-1 ring-green-500" : "border-red-500 ring-1 ring-red-500"}`,
                  }),
                ),
              ),
              (!settings.semester || settings.semester === "Ganjil") &&
                React.createElement(
                  ...
                  React.createElement("input", {
                    ...
                    className: `w-full p-0.5 text-center text-[10px] border rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500 ${weights.STS !== null && weights.STS !== undefined && weights.STS !== "" ? "border-green-500 ring-1 ring-green-500" : "border-red-500 ring-1 ring-red-500"}`,
                  }),
                ),
              settings.semester === "Genap" &&
                React.createElement(
              ...
              (!settings.semester || settings.semester === "Ganjil") &&
                React.createElement(
                  ...
                  React.createElement("input", {
                    ...
                    className: `w-full p-0.5 text-center text-[10px] border rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500 ${weights.SAS !== null && weights.SAS !== undefined && weights.SAS !== "" ? "border-green-500 ring-1 ring-green-500" : "border-red-500 ring-1 ring-red-500"}`,
                  }),
                ),
              settings.semester === "Genap" &&
                React.createElement(
        ...
        React.createElement(
          ...
          relevantStudents.map((student, index) => {
            ...
            let total = null;
            if (detailedGrade) {
              total = studentGrade.finalGrades?.[subject.id];
            }
            ...
            return React.createElement(
              ...
              React.createElement(
                ...
                {
                  className:
                    "p-1 border-b border-l border-slate-200 w-20 min-w-[5rem] text-center font-bold align-top pt-3",
                },
  ...
  return React.createElement(
    ...
    students.length === 0
      ? React.createElement(
          ...
          {
            className:
              "bg-white p-10 rounded-xl shadow-md border border-slate-200 text-center min-h-[400px] flex items-center justify-center",
          },
          React.createElement(
            "p",
            { className: "text-slate-500" },
      ...
      : React.createElement(
          ...
          React.createElement(
            ...
            { className: "pt-4" },
            activeTab === "keseluruhan"
              ? React.createElement(NilaiKeseluruhanView, props)
              : renderSubjectView(),
          ),
        ),
  );
