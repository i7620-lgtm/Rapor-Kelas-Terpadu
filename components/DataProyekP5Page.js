                    ...
                    React.createElement('div', { className: "space-y-8" },
                        ...
                        React.createElement('div', { className: "bg-white p-6 rounded-xl shadow-md border border-slate-200" },
                            ...
                            subElementsForTable.length > 0 ? (
                                React.createElement('div', { className: "overflow-x-auto" }, React.createElement('table', { className: "w-full text-sm text-left text-slate-500 border-collapse" }, React.createElement('thead', null, React.createElement('tr', { className: "bg-slate-100" }, React.createElement('th', { scope: "col", className: "px-6 py-3 border border-slate-200 sticky left-0 bg-slate-100 z-10 w-64" }, "Nama Siswa"), ...subElementsForTable.map(sub => (React.createElement('th', { key: sub.key, scope: "col", className: "px-4 py-3 border border-slate-200 w-48 text-center text-xs text-slate-700" }, React.createElement('div', { className: "font-bold" }, sub.dimension), React.createElement('div', { className: "font-normal text-slate-500" }, sub.name))))), React.createElement('tbody', null, students.map(student => {const studentAssessments = assessments.find(a => a.studentId === student.id && a.projectId === activeProject.id); return (React.createElement('tr', { key: student.id, className: "bg-white hover:bg-slate-50" }, React.createElement('th', { scope: "row", className: "px-6 py-2 border border-slate-200 font-medium text-slate-900 whitespace-nowrap sticky left-0 bg-white z-10 w-64" }, student.namaLengkap), ...subElementsForTable.map(sub => (React.createElement('td', { key: sub.key, className: "p-1 border border-slate-200" }, React.createElement('select', { value: studentAssessments?.assessments[sub.key] || '', onChange: (e) => handleAssessmentChange(student.id, sub.key, e.target.value), className: "w-full h-full p-2 border-0 focus:ring-1 focus:ring-indigo-500 text-xs rounded-md", "aria-label": `Penilaian untuk ${student.namaLengkap} pada ${sub.name}` }, React.createElement('option', { value: "" }, "-"), ...ASSESSMENT_LEVELS.map(level => (React.createElement('option', { key: level, value: level }, level))))))))}))))
